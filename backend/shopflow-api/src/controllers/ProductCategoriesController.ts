...
router.get("/product-categories", verifyToken, async (req: Request, res: Response) => {
    try {
        const repo = AppDataSource.getRepository(ProductCategories);
...
router.put("/product-categories/:id", verifyToken, async (req: Request, res: Response) => {
    try {
        ...
        if (!category) return res.status(404).json({ mensagem: "Não encontrada." });
        ...
        return res.status(200).json({ mensagem: "Atualizado.", category });
...
router.post("/product-categories", verifyToken, async (req: Request, res: Response) => {
    try {
        ...
        return res.status(201).json({ mensagem: "Criado.", category: novo });
    } catch (error) {
        if (error instanceof yup.ValidationError) return res.status(400).json({ mensagem: "Erro validação", erros: error.inner });
        return res.status(500).json({ mensagem: "Erro interno." });
    }
